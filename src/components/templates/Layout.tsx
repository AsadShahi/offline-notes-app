...
export default function Layout({ children }: LayoutProps) {
  ...
  // Force dark mode sync with HTML element
  useEffect(() => {
    ...
    <div className={`flex h-screen ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 '    } text-gray-900 dark:text-gray-100`}>
      ...
      <div className={`hidden md:flex ${isDark ? 'bg-gray-800   border-gray-700 text-white' : 'bg-white border-gray-200 '} w-64 flex-col border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800`}>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700">
          <button
            onClick={toggleTheme}
            className="flex items-center justify-center w-full px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors"
          >
            {isDark ? (
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex justify-around p-2">
          <Link 
            ...
            className={`flex flex-col items-center p-2 rounded-lg ${
              isActive('/notes') 
              ? 'bg-primary-100 dark:bg-primary-900 text-primary-900 dark:text-primary-100' 
              : `{   ${isDark?'hover:bg-gray-700 text-gray-100':'hover:bg-gray-100 text-gray-900' }`}
            }`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 20 20" fill="currentColor">
              <path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z" />
            </svg>
            <span className="text-xs mt-1 text-gray-900">Notes</span>
          </Link>
          <Link 
            href="/favorites" 
            className={`flex flex-col items-center p-2 rounded-lg ${
